...
@Route("friendly-captcha")
public class FriendlyCaptchaView extends VerticalLayout {
    ...
    public FriendlyCaptchaView(@Value("${captcha.friendlycaptcha.site-key}") String siteKey,
        ...
        VerticalLayout content = new VerticalLayout(new H1("Friendly Captcha"), formContainer);
    ...
    private void buildForm() {
        ...
        captcha.addTokenListener(event -> {
            VerificationResult result = new FriendlyCaptchaVerifier(secret, siteKey).verify(event.getToken());
            if (result.success()) {
